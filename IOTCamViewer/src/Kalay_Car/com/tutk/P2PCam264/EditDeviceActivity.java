...
import com.tutk.P2PCam264.object.Convert;
...
public class EditDeviceActivity extends Activity implements IRegisterIOTCListener, OkCancelDialogListener {
    private static String TAG = EditDeviceActivity.class.getSimpleName();
    ...
    private byte alarm_motion_armed = 0;
    ...
    //
    private View motiondetect;
    private TextView motiondetectf;
    private boolean motio = false;
    private Switch motiondetects;
    private View gsensor;
    private TextView gsensorf;
    private Spinner gsensors;
    private int gvalue = 0;
    private View timesync;
    private TextView timesyncf;
    private ImageButton timesynce;
    private RelativeLayout timesyncl;
    private Button timesyncb;
    private View factory_default;
    private TextView factory_defaultf;
    private ImageButton factory_defaulte;
    private RelativeLayout factory_defaultl;
    private Button factory_defaultb;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        motiondetect = (View) findViewById(R.id.motiondetect);
        motiondetect.setVisibility(View.GONE);
        motiondetectf = (TextView) motiondetect.findViewById(R.id.description);
        motiondetectf.setText(R.string.motiondetect);
        motiondetects = (Switch) motiondetect.findViewById(R.id.opstatus);

//        motiondetects.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
//            @Override
//            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
//                Log.d(TAG, "CheckedChanged : " + isChecked);
//                Log.d(TAG, "State  : " + motiondetects.isChecked());
//                String req = "custom=1&cmd=2006&par=" + 0;//FIXME not work ?!!!
//                if (isChecked == motio){
//                    return;
//                }
//
////                motiondetects.setChecked(isChecked);
//
//
//                if (isChecked) {
//                    Log.d(TAG,"YES");
//                    req = "custom=1&cmd=2006&par=" +1;//FIXME not work ?!!!
...
//                            (0, 0, 0, 2006, 1, 0, req.length(), req));
//                    Log.d(TAG, "custom command  : " + req);
////                    mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SETGUARD_REQ, AVIOCTRLDEFs.SMsgAVIoctrlSetGuardReq.parseContent(Camera.DEFAULT_AV_CHANNEL,
////                            (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0));
//                } else {
//                    Log.d(TAG,"OFF");
//                    req = "custom=1&cmd=2006&par=" +0;//FIXME not work ?!!!
////                    mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SETGUARD_REQ, AVIOCTRLDEFs.SMsgAVIoctrlSetGuardReq.parseContent(Camera.DEFAULT_AV_CHANNEL,
...
//        });
        ...
        motiondetects.setOnCheckedChangeListener(null);
        ...
        motiondetects.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SETMOTIONDETECT_REQ,
                        AVIOCTRLDEFs.SMsgAVIoctrlSetMotionDetectReq.parseContent(mDevice.ChannelIndex, sen));
        ...
        gsensor = (View) findViewById(R.id.gsensor);
        gsensor.setVisibility(View.GONE);
        gsensorf = (TextView) gsensor.findViewById(R.id.description);
        gsensorf.setText(R.string.gsnsor);
        gsensors = (Spinner) gsensor.findViewById(R.id.spinner_backoff);
        gsensors.setAdapter(new BaseAdapter() {
            String[] mTexts = EditDeviceActivity.this.getResources().getStringArray(R.array.gsensor_texts);
            int[] mValues = EditDeviceActivity.this.getResources().getIntArray(R.array.gsensor_values);
        ...
        gsensors.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String req = "custom=1&cmd=2011&par=" + position;//FIXME not work ?!!!
                if (position == gvalue) {
                    return;
                }
                if (gvalue < 4) {
        ...
        timesync = (View) findViewById(R.id.timesync);
        timesyncf = (TextView) timesync.findViewById(R.id.function);
        timesyncf.setText(R.string.timesync);
        timesynce = (ImageButton) timesync.findViewById(R.id.expand);

        timesyncl = (RelativeLayout) timesync.findViewById(R.id.layout_expand);
        timesynce.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (timesyncl.getVisibility() == View.GONE) {
                    timesyncl.setVisibility(View.VISIBLE);
                } else {
                    timesyncl.setVisibility(View.GONE);
                }
            }
        });

        timesyncb = (Button) timesync.findViewById(R.id.btnfunction);
        timesyncb.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
...
//                    rd = AVIOCTRLDEFs.AVIOTC_RECORDTYPE_MANUAL;
                ...
                mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SETRECORD_REQ,
                        AVIOCTRLDEFs.SMsgAVIoctrlSetRecordReq.parseContent(mDevice.ChannelIndex, AVIOCTRLDEFs.AVIOTC_RECORDTYPE_OFF));
                final String[] alltime = sFull.format(System.currentTimeMillis()).split("_");
//                Log.d(TAG, "Time : " + alltime[0]);
                String req = "custom=1&cmd=3005&str=" + alltime[0];
                mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_WIFICMD_REQ, AVIOCTRLDEFs.SMsgAVIoctrlWifiCmdReq.parseContent
                        (0, 0, 0, 3005, 1, 0, req.length(), req));
                Log.d(TAG, "custom command  : " + req);
                req = "custom=1&cmd=3006&str=" + alltime[1];
                mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_WIFICMD_REQ, AVIOCTRLDEFs.SMsgAVIoctrlWifiCmdReq.parseContent
                        (0, 0, 0, 3006, 1, 0, req.length(), req));
                Log.d(TAG, "custom command  : " + req);

                req = "custom=1&cmd=9090";
                mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_WIFICMD_REQ, AVIOCTRLDEFs.SMsgAVIoctrlWifiCmdReq.parseContent
                        (0, 0, 0, 9090, 1, 0, req.length(), req));
                Log.d(TAG, "custom command  : " + req);
            }
        });

        timesyncb.setText(R.string.timesync);

        factory_default = (View) findViewById(R.id.factory_default);
        factory_defaultf = (TextView) factory_default.findViewById(R.id.function);
        factory_defaultf.setText(R.string.factory_default);
        factory_defaulte = (ImageButton) factory_default.findViewById(R.id.expand);
        factory_defaultl = (RelativeLayout) factory_default.findViewById(R.id.layout_expand);
        factory_defaulte.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (factory_defaultl.getVisibility() == View.GONE) {
                    factory_defaultl.setVisibility(View.VISIBLE);
                } else {
                    factory_defaultl.setVisibility(View.GONE);
                }
            }
        });
        factory_defaultb = (Button) factory_default.findViewById(R.id.btnfunction);
        factory_defaultb.setText(R.string.factory_default);
        factory_defaultb.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                String req = "custom=1&cmd=3011";//FIXME not work ?!!!

                mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_WIFICMD_REQ, AVIOCTRLDEFs.SMsgAVIoctrlWifiCmdReq.parseContent
                        (0, 0, 0, 3011, 1, 0, req.length(), req));
                Log.d(TAG, "custom command  : " + req);
...
//                Log.d(TAG, "custom command  : " + req);
        ...
        if (mCamera != null && mCamera.isChannelConnected(0)) {
            ...
            String req = "custom=1&cmd=2011&par=" + 0;//FIXME not work ?!!!
            req = "custom=1&cmd=3014";
            mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_WIFICMD_REQ, AVIOCTRLDEFs.SMsgAVIoctrlWifiCmdReq.parseContent
                    (0, 0, 0, 3014, 1, 0, req.length(), req));
            Log.d(TAG, "custom command  : " + req);
...
//                motiondetect.setVisibility(View.VISIBLE);
                motiondetect.setVisibility(View.GONE);
            }
...
//                    Toast.makeText(mActivity, getString(R.string.setting_ok), Toast.LENGTH_SHORT).show();
...
//                    Log.d(TAG," IOTYPE_USER_IPCAM_GETGUARD_RESP : "+ data[4]);
//                    motiondetects.setChecked(data[4] == (byte) 1);
                ...
                case AVIOCTRLDEFs.IOTYPE_USER_IPCAM_GETMOTIONDETECT_RESP:
                    ...
                    Log.d(TAG, " SHow MOTIONDETECT_RESP : " + mSensitivity);
                    if (mSensitivity < 1) {
                        motio = false;
                    } else {
                        motio = true;
                    }
                    Log.d(TAG, "Motion State : " + motio);
                    motiondetects.setChecked(motio);
                    btnEvent.setEnabled(true);
                    tvEvent.setTextColor(Color.BLACK);
                    break;
                ...
                case AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SETMOTIONDETECT_RESP:
                    ...
                    Log.d(TAG, "IOTYPE_USER_IPCAM_SETMOTIONDETECT_RESP    ");
//                    Toast.makeText(mActivity, getString(R.string.setting_ok), Toast.LENGTH_SHORT).show();
                    break;
...
//                    Toast.makeText(mActivity, getString(R.string.setting_ok), Toast.LENGTH_SHORT).show();
                    ...
                    String[] filter = new String(Convert.byteArray2String(buff)).split(",");
                    Log.d(TAG, "Get WiFi Command Response CMDID :  " + AVIOCTRLDEFs.SMsgAVIoctrlWifiCmdResp.nCmdID);
//                Log.d(TAG, "Get WiFi Command Response CMDID :  " + AVIOCTRLDEFs.SMsgAVIoctrlWifiCmdResp.nCmdID + "__ " + new String(buff));
                    Log.d(TAG, "Splite Response  :  " + Convert.byteArray2String(buff));
                    String[] ii = Convert.byteArray2String(buff).split("\n");
                    final int count = ii.length;
                    Log.d(TAG, " Wifi re size : " + ii.length);
                    for (int i = 0; i < count; ++i) {
                        Log.d(TAG, " array info  : " + ii[i]);
                        if (ii[i].contains("2011")) {
                            int o = Integer.valueOf(ii[i].split(",")[1]);
                            gvalue = o;
                            gsensors.setSelection(o, false);
                        }
//                        if (ii[i].contains("2006")){
//                            int o =  Integer.valueOf(ii[i].split(",")[1]);
//                            if (o==1){
//                                motio =true;
//                            }
//                            Log.d(TAG, "motion : "+motio);
//
//                        }
                    }
